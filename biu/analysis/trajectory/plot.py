...
def _trajectoryPlot1D_super(pdata, conditions, component, cmap, ax, pca_var_explained=None):
    """
...
def _trajectoryPlot1D_grid(pdata, conditions, component, cmap, ax, pca_var_explained=None):
    """
    Internal function for trajectoryPlot. Plots in 1D
    Inputs:
        pdata: DataFrame. A pivoted table with multilevel index on columns: components -> conditions
    ...
    for i in range(0, sorted_pdata.shape[0]):
        ...
        ax.plot(xvals, yvals, linestyle='-', c='k', zorder=1)
    ...
    return sorted_pdata, ax
#edef

def _trajectoryPlot1D_embed(pdata, conditions, component, cmap, ax, pca_var_explained=None):
    """
...
def _trajectoryPlot2D_super(pdata, conditions, component1, component2, cmap, ax, pca_var_explained=None):
    """
...
def _trajectoryPlot2D_grid(pdata, conditions, component1, component2, cmap, ax, pca_var_explained=None):
    """
    ...
    Outputs:
        pdata and axis
    """
    pdata = ops.dataframe.reorder(pdata)
...
def _trajectoryPlot2D_embed(pdata, conditions, component1, component2, cmap, ax, pca_var_explained=None):
    """
...
def plot(data, labels=None, conditions=None, meas=None, samples=None, pca=True, dim=2, 
         col_index='index', col_condition='condition',
         component=0, component1=0, component2=1, ax=None, cmap=plt.get_cmap('Set2'),
         display_1d='grid', display_2d='super'):
    """
    ...
    Inputs:
        data, labels: Either-
          ...
          * data:   The dataframe of expression/measurement counts. Rows are samples, Genes/Measurements are columns
            labels: The dataframe of information about the samples. Must have same index as expr dataframe.
        ...
        meas:       Which measures to use (i.e. columns in the data matrix)
    ...
    Output:
        A namedtuple, with values:
          * pdata:   The plotted data
          * ax:      The matplotlib axisn plotted to
          * pca_fit: The PCA fit that was made
    ...
    if meas is None:
        meas = data.columns.values
    #fi
    
    if conditions is None:
        conditions = sorted(labels[col_condition].drop_duplicates().values)
    #fi
    
    if samples is None:
        samples = labels[col_index].drop_duplicates()
    #fi
    ...
    pca_fit           = None
    if pca:
        data, pca_fit = ops.dataframe.pca(data, nc=max(component, component1, component2)+1, ret_fit=True)
        pca_var_explained = { 
    ...
    pdata = data.join(labels[[col_index, col_condition]])
    pdata = pdata.pivot(index=col_index, columns=col_condition, values=list(data.columns))
    pdata = pdata.loc[samples]
    ...
    if dim == 1:
        ...
        rpdata, ax = funcs[display_1d.lower()](pdata, conditions, component, cmap, ax, pca_var_explained)
    else:
        funcs = { 'super' : _trajectoryPlot2D_super,
                  'grid':   _trajectoryPlot2D_grid,
                  'embed' : _trajectoryPlot2D_embed}
        rpdata, ax = funcs[display_2d.lower()](pdata, conditions, component1, component2, cmap, ax, pca_var_explained)
    #fi
    
    nt = namedtuple("trajectoryPlot_data", [ 'pdata', 'ax', 'pca_fit' ])
    return nt(rpdata, ax, pca_fit)
