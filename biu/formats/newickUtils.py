# Modified from https://raw.githubusercontent.com/glottobank/python-newick/master/src/newick.py
...
class Newick(object):
  ...
  def __str__(self):
    ...
    dstr += " Trees: %d\n" % len(self.trees)
    if len(self.trees) == 1:
      dstr += " Nodes: %d\n" % len([ n for n in self.walk() ])
      names = self.get_leaf_names()
      dstr += " Leaves: %d\n" % len(names)
    #fi
    return dstr
  #edef
...
class Node(object):
    ...
    def __repr__(self):
        dstr = 'NewickNode( %s : %s [%d children])' % (self.name, '?' if self._length is None else str(self._length), len(self.descendants))
        return dstr
    #edef

    def __getitem__(self, c):
      if isinstance(c, str):
        return self.get_node(c)
      else:
    ...
    def get_leaves(self):
        ...
        """
        return Newick([n for n in self.walk() if n.is_leaf])
    ...
    def name_internal(self):
      ...
      for node in self.walk():
        if (not node.is_leaf) and (node.name is None):
          node.setName("internal_%d" % (internal_i))
          internal_i += 1
        #fi
      #efor
    #edef
    ...
    def remove_names(self):
        ...
        """
        self.visit(lambda n: n.setName(None))

    def remove_internal_names(self):
        """
        Set the name of all non-leaf nodes in the subtree to None.
        """
        self.visit(lambda n: n.setName(None), lambda n: not n.is_leaf)
